...
class Classifier:
    ...
    async def predict(self, vehicle_id, timestamp):
        ...
        dep_uri = response['stops']['stop'][0]['stationinfo'][
            '@id']  # Name of the departure station
        stop_dep = int(
            self._stations[self._stations['URI'] == dep_uri].iloc[0, :]
            ['avg_stop_times']
        )
        arr_uri = response['stops']['stop'][-1]['stationinfo']['@id']
        stop_arr = int(
            self._stations[self._stations['URI'] == arr_uri].iloc[0, :]
            ['avg_stop_times']
        )
        ...
        for stop in response['stops']['stop']:
            try:
                ...
                station_info = self._stations[self._stations['URI'] ==
                                              station_cur_uri].iloc[0, :]
                ...
                vector = [
                    station_cur,
                    str(dotw), weekend, month, seconds_since_midnight,
                    expected_time_station, station_lng, station_lat,
                    station_stop, train_type, line_id, stop_arr, stop_dep, line
                ]
                delay = await IOLoop.current().run_in_executor(
                    None, self._cat.predict, [vector]
                )
                delay = delay[
                    0
                ]  # Read after await, we want to get the first item of the result, not of a Future object
    ...
    async def _get_vehicle(self, vehicle_id, timestamp):
        ...
        try:
            response = await http_client.fetch(
                VEHICLE_API_URL.format(
                    vehicle_id, timestamp.strftime('%d%m%y')
                )
            )
